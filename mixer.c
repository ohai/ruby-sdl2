...
static VALUE Mixer_s_play_channel(VALUE self, VALUE channel, VALUE chunk, VALUE loops)
{
    HANDLE_MIX_ERROR(Mix_PlayChannel(NUM2INT(channel), Get_Mix_Chunk(chunk),
                                     NUM2INT(loops)));
    return Qnil;
}
...
void rubysdl2_init_mixer(void)
{
    ...
    rb_define_module_function(mMixer, "play_channel", Mixer_s_play_channel, 3);
